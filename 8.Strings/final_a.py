...
---ПРИНЦИП РАБОТЫ---
    ...
    префикса. Для этого берем в качестве эталона первое слово из списка распакованных
    слов, его длину записываем в переменную k, которая обозначает, до какого символа
    ...
    по символ с индексом k.  Kак только символы расходятся, то выхожу из цикла и записываю
    текущий индекс плюс коэффициент (0 или 1) в переменную k (коэффициент нужен так как в
    моей реализации при полном совпадении эталона с префиксом строки и при несовпадении
    последнего символа эталона индексы по выходу из цикла будут одинаковы, потому для
    различения этих двух случаев вводится флаг interrupt и от его значения по выходе
    из цикла коэффициент принимает нужное значение). Получившийся после прохождения всех
    слов из списка k и есть длина общего для всех слов префикса.
...
---ПРОСТРАНСТВЕННАЯ СЛОЖНОСТЬ---
    Для хранения распакованных строк понадобится список разменром O(n), где n - количество слов.
    ...
    При нахождении общего префикса нам потребуется хранить эталон размером O(k), где k - длина
    первого распакованного слова.
    Итоговая пространственная сложность алгоритма составит O(n + m + k)
"""
    ...
    for i in range(1, len(pw)):
        ...
        for j in range(max_prefix_len):
            if word[j] != standard[j]:
                interrupt = True
                break
        k = 0 if interrupt else 1
        max_prefix_len = j + k
    return ''.join(standard[:max_prefix_len])
