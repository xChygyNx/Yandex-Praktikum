...
--- ВРЕМЕННАЯ СЛОЖНОСТЬ --- 
    ...
    Считывание и сохранение документов занимает O(n * u1), где n - количество документов,
    u1 - среднее количество уникальных слов в документе; процесс подсчета релевантности
    O(n * m * u2),где n - количество документов, m - количество запросов, u2 - среднее количество
    уникальных слов в запросе. Дальше происходит сортировка полученного списка релевантности
    запроса документам. В Python метод sort использует алгоритм Тима, средняя временная
    сложность которого равна O(n * log n), отсортированный список усекается до 5 первых
    элементов за O(1).

--- ПРОСТРАНСТВЕННЯ СЛОЖНОСТЬ --- 
    Хранение словарей с документами O(n * u1) дополнительной памяти, где
    n - количество документов, u1 - количество уникальных слов в документе. При подсчете релевантности
    для каждого запроса создается множество слов, из которых он состоит, что требует еще O(m*u2),
    где m - количество запросов, u2 - среднее количество уникальных слов в запросе. Так же в процессе
    подсчета создается словарь tmp, который в худшем случае будет занимать O(n) места (если слова из
    запроса присутствуют во всех документах), таких словарей создано m штук (для каждого запроса),
    следовательно суммарная пространственная сложность данного этапа составит O(n*m).
    Результирующий список еще в среднем O(5m) -> O(m). Итоговая пространственная сложность составляет
    O((n * u1) + (m * u2) + 5m + (n * m))
"""
